...
class Session:
    ...
    def train(self, epoch,train_images,train_tags,train_labels,train_events,
        ...
        for idx, (img, txt, events, labels) in enumerate(self.train_loader):
            ...
            self.opt_model1.zero_grad()
            loss_fn = nn.CrossEntropyLoss()
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

            loss,pred,fusion_teacher,code_I,code_T = self.model1(img,txt,labels,events,'train')

            loss3 = loss
            loss3.backward()
            # loss.backward()
            # self.opt_model.step()
            self.opt_model1.step()
    ...
    def eval(self,step,  last=False):
        ...
        for idx, (img, txt, event,labels) in enumerate(self.test_loader):
            ...
            txt = torch.FloatTensor(txt_feature.numpy()).cuda()
            img = torch.FloatTensor(img.numpy()).cuda()
            loss,test_label_pred,fusion,code_I,code_T = self.model1(img,txt,labels,event,'test')
            pre_label_detection = test_label_pred.argmax(1)
...
def main():
    for x in range(1):
        ...
        train_labels = np.load(
            ...
            allow_pickle=True)
        index = np.load("/home/user01/hdd/yxf/完整版半监督6.8/weiboindex20.2.npy")
        index.sort()
        labeled_index = np.zeros(1202, dtype=int)
        unlabeled_index = np.zeros(4807, dtype=int)
        a = 0
        b = 0
        symbol = 0
        # 获得未标记数据下标
