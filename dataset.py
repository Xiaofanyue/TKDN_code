...
class Dataset1(torch.utils.data.Dataset):
    def __init__(self, images, tags, labels,event, test=None):
        self.test = test
        ...
        self.event_label = torch.from_numpy(np.array(event))

    def __len__(self):
        return len(self.train_labels)

    def __getitem__(self, idx):
        return self.train_images[idx], self.train_tags[idx],self.event_label[idx] , self.train_labels[idx]
